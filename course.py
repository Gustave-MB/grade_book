...
class Course:
    ...
    def add_course_data_from_file(self, file_name):
        """
        Description: add_course_data_from_file takes the filename (file path) aand extract the course 
        details including course ID, course name, course instructor,semester and course TAs
        Parameters
        ----------
        file_name : file path
        """
        # opening filename in read mode using context manager
        with open(file_name,'r') as filereader:
            # read the first 5 lines one by one and stores them in their respective variables
             course_id=filereader.readline().strip('\n')
             course_name=filereader.readline().strip('\n')
             semester=filereader.readline().strip('\n')
             instructor=filereader.readline().strip('\n')
             TAs=filereader.readline().strip('\n')
             # set the course data
             self.set_course_id(course_id)
             self.set_course_name(course_name)
             self.set_instructor(instructor)
             self.set_semester(semester)
             self.set_courseTAs(TAs)
...
course2.add_students_from_file('data.txt')
